...
$('.selectOffer .cta').click(function() {
    var parentTop = $(this).parent().parent().parent().parent().parent().parent().parent();
    
    if ( !$(this).hasClass('red')) {
        $('.selectedBox').remove();
        $('.selectOffer .cta.red').find('span').replaceWith('<span>Select Offer</span>');
        $('.selectOffer .cta').addClass('green').removeClass('red');
        $('.overlayButton').css('display', 'none');
        $('.overlayContinue').css('display', 'none');
    }
    
    if ( $(this).hasClass('red')) {
        $(this).addClass('green').removeClass('red');
        $(this).find('span').replaceWith('<span>Select Offer</span>');
        $(parentTop).find('.packageHead .packageTitle .selectedBox').remove();
        $(parentTop).find('.overlayButton').css('display', 'none');
        $(parentTop).find('.overlayContinue').css('display', 'none');
        $('.nextStep a').css({'pointer-events': 'none', 'color': '#99999b'});
    } else {
        $(this).addClass('red').removeClass('green');
        $(this).find('span').replaceWith('<span>Remove Offer</span>');
        $(parentTop).find('.packageHead .packageTitle').prepend('<div class="selectedBox"><div class="icon-only icon-ok"><span></span></div></div>');
        $(parentTop).find('.overlayButton').css('display', 'block');
        $(parentTop).find('.overlayContinue').css('display', 'block');
        $('.nextStep a').css({'pointer-events': 'auto', 'color': '#3376b2'});
    }
    return false;
});
$('.iconRemove').click(function() {
    var parentTop = $(this).parent().parent().parent();
    $('.selectedBox').remove();
    $('.selectOffer .cta.red').find('span').replaceWith('<span>Select Offer</span>');
    $('.selectOffer .cta').addClass('green').removeClass('red'); 
    $(parentTop).find('.overlayButton').css('display', 'none');

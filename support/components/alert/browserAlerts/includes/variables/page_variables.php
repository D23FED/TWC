<?php 

// Set Page variable values
	
	// header.php variables
		// Page Title
		$title = 'Support Browser Alert';
	
		// Page Description
		$description = '';
	
		// Page Specific Keywords
		$keywords = '';
		
		// Body ID tag
		$body_ID = '';
	
	// alert.php variables
		// Alert, if needed for demo
		$alert = 'This is a test alert';
		
		$browserAlerts = '<b>Cookies are not enabled:</b>You do not have cookies enabled. In order to serve you better, please enable cookies.';

	// subnav active variable
		$subNavSection = 'Support';

	// primary breadcrumb
		$breadcrumb_A = '';
		
	// secondary breadcrumb
		$breadcrumb_B = '';
		
	// secondary breadcrumb
		$breadcrumb_C = '';
			
	// banner.php variables
		// Banner Title - H1 tag
		$banner_title = '';
		
	// content.php variables
		// SubHead text
		$subHead = '';

// Setup the page
			
		$browserAlerts = true;	
				
		$tophat = false;
		
		$logoHeader = false;
		
		$nav = false;
		
		$subNav = false;
		
		$alert = false;
		
		$breadcrumbs = false;
		
		$banner = false;
		
		$supportSearch = false;
			
		$content = false;
		
		$footer = false;
		
?>
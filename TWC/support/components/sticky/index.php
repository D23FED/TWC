<?
if($_SERVER['REQUEST_URI'] !== '/TWC/styleguide/') {
  include($_SERVER['DOCUMENT_ROOT']. '/TWC/core/includes/header.php');
}
?>


<div class="twc-box-column">
  <div class="100 section columnControl">
    <div class="parsys_column twc-col1_100">
      <div class="parsys parsys0 twc-col1_100-c0 parsys_column">
        <div class="parbase section">

          <div class="60-40 section columnControl">
            <div class="parsys_column twc-col2_6040">
              <div class="parsys parsys0 twc-col2_6040-c0 parsys_column">
                <div class="parbase section">
                  <h3>Headline for News Article</h3>
                  <p>Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. Donec eu libero sit amet quam egestas semper. Aenean ultricies mi vitae est. Mauris placerat eleifend leo. Quisque sit amet est et sapien ullamcorper pharetra. Vestibulum erat wisi, condimentum sed, commodo vitae, ornare sit amet, wisi. Aenean fermentum, elit eget tincidunt condimentum, eros ipsum rutrum orci, sagittis tempus lacus enim ac dui. <a href="">Tech Timeline</a></p>
                  <p>Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. Donec eu libero sit amet quam egestas semper. Aenean ultricies mi vitae est. Mauris placerat eleifend leo. Quisque sit amet est et sapien ullamcorper pharetra. Vestibulum erat wisi, condimentum sed, commodo vitae, ornare sit amet, wisi. Aenean fermentum, elit eget tincidunt condimentum, eros ipsum rutrum orci, sagittis tempus lacus enim ac dui.</p>
                </div>
                <div class="new section"></div>
              </div>
              <div class="parsys parsys1 twc-col2_6040-c1 parsys_column">
                <div class="parbase section">
                  <figure>
                    <img alt="important people at an important event" src="images/Formal_Affair.jpg">
                  </figure>
                </div>
                <div class="new section"></div>
              </div>
            </div>
            <div class="columnClear"></div>
          </div>

        </div>
      <div class="new section"></div>
      </div>
    </div>
    <div class="columnClear"></div>
  </div>
</div>

<div class="choose-services sticky">
  <div class="70-30 section columnControl">
    <div class="parsys_column twc-col2_7030">
      <div class="parsys parsys0 twc-col2_7030-c0 parsys_column">
        <div class="parbase section">
          <h4>
            <div class="text parbase twc-plain-text instructions-1">
              Choose the services you want.
            </div>
          </h4>
          <p>
            <div class="text parbase twc-plain-text instructions-2">
              Select the plans you want to order. Then set up your services by clicking "Order Now"
            </div>
          </p>
          <h4>
            <div class="text parbase twc-plain-text instructions-1">
            SPECIAL OFFER: FREE Shipping on Easy Connect Kits
            </div>
          </h4>
        </div>
        <div class="new section"></div>
      </div>
      <div class="parsys parsys1 twc-col2_7030-c1 parsys_column">
        <div class="parbase section">
          <div class="sticky-pricing">
            <div class="50-50 section columnControl">
              <div class="parsys_column twc-col2_5050">
                <div class="parsys parsys0 twc-col2_5050-c0 parsys_column">
                  <div class="parbase section">
                    <div class="pricing">
                      <span class="h4">
                        <sup class="dollar-sign">$</sup>
                        <span class="dollars">79</span>
                        <sup class="cents">99</sup>
                      </span>
                      <span class="term">Per mo.<br>for 12 mos.</span>
                    </div>
                  </div>
                  <div class="new section"></div>
                </div>
                <div class="parsys parsys1 twc-col2_5050-c1 parsys_column">
                  <div class="parbase section">
                    <div class="cta omega icon-after icon-angle-right">
                      <a href="#">
                        <span>Order Now</span>
                      </a>
                    </div>
                  </div>
                  <div class="new section"></div>
                </div>
              </div>
              <div class="columnClear"></div>
            </div>
          </div>
        </div>
        <div class="new section"></div>
      </div>
    </div>
    <div class="columnClear"></div>
  </div>

</div>


<div class="100 section columnControl">
  <div class="parsys_column twc-col1_100">
    <div class="parsys parsys0 twc-col1_100-c0 parsys_column">
      <div class="parbase section">
        <h3>Sed Ut Perspiciatis</h3>

        <div class="25-25-25-25 section columnControl">
          <div class="parsys_column twc-col4_25252525">

            <div class="parsys parsys0 twc-col4_25252525-c0 parsys_column">
              <div class="parbase section">
                <div class="bottom-box">
                  <figure>
                    <img alt="TWC_logo-placeholder" src="/TWC/core/images/TWC_logo-placeholder.png">
                  </figure>
                  <h5>People</h5>
                  <p>Liusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud</p>
                  <div class="cta omega twc-icon-after icon-angle-right mobile-full">
                              <a href="http://google.com" analyticsname="test">
                                  <span>Learn More</span>
                              </a>
                          </div>
                </div>
              </div>
              <div class="new section"></div>
            </div>

            <div class="parsys parsys1 twc-col4_25252525-c1 parsys_column">
              <div class="parbase section">
                <div class="bottom-box">
                  <figure>
                    <img alt="TWC_logo-placeholder" src="/TWC/core/images/TWC_logo-placeholder.png">
                  </figure>
                  <h5>Apps</h5>
                  <p>Liusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud</p>
                  <div class="cta omega twc-icon-after icon-angle-right mobile-full">
                              <a href="http://google.com" analyticsname="test">
                                  <span>Learn More</span>
                              </a>
                          </div>
                </div>
              </div>
              <div class="new section"></div>
            </div>

            <div class="parsys parsys1 twc-col4_25252525-c2 parsys_column">
              <div class="parbase section">
                <div class="bottom-box">
                  <figure>
                    <img alt="TWC_logo-placeholder" src="/TWC/core/images/TWC_logo-placeholder.png">
                  </figure>
                  <h5>TWC History</h5>
                  <p>Liusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud</p>
                  <div class="cta omega twc-icon-after icon-angle-right mobile-full">
                              <a href="http://google.com" analyticsname="test">
                                  <span>Learn More</span>
                              </a>
                          </div>
                </div>
              </div>
              <div class="new section"></div>
            </div>

            <div class="parsys parsys1 twc-col4_25252525-c3 parsys_column">
              <div class="parbase section">
                <div class="bottom-box">
                  <figure>
                    <img alt="TWC_logo-placeholder" src="/TWC/core/images/TWC_logo-placeholder.png">
                  </figure>
                  <h5>TWC Research</h5>
                  <p>Liusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud</p>
                  <div class="cta omega twc-icon-after icon-angle-right mobile-full">
                              <a href="http://google.com" analyticsname="test">
                                  <span>Learn More</span>
                              </a>
                          </div>
                </div>
              </div>
              <div class="new section"></div>
            </div>
          </div>
          <div class="columnClear"></div>
        </div>


      </div>
    <div class="new section"></div>
    </div>
  </div>
  <div class="columnClear"></div>
</div>

<div class="twc-box-column">
  <div class="100 section columnControl">
    <div class="parsys_column twc-col1_100">
      <div class="parsys parsys0 twc-col1_100-c0 parsys_column">
        <div class="parbase section">

          <div class="60-40 section columnControl">
            <div class="parsys_column twc-col2_6040">
              <div class="parsys parsys0 twc-col2_6040-c0 parsys_column">
                <div class="parbase section">
                  <h3>Headline for News Article</h3>
                  <p>Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. Donec eu libero sit amet quam egestas semper. Aenean ultricies mi vitae est. Mauris placerat eleifend leo. Quisque sit amet est et sapien ullamcorper pharetra. Vestibulum erat wisi, condimentum sed, commodo vitae, ornare sit amet, wisi. Aenean fermentum, elit eget tincidunt condimentum, eros ipsum rutrum orci, sagittis tempus lacus enim ac dui. <a href="">Tech Timeline</a></p>
                  <p>Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. Donec eu libero sit amet quam egestas semper. Aenean ultricies mi vitae est. Mauris placerat eleifend leo. Quisque sit amet est et sapien ullamcorper pharetra. Vestibulum erat wisi, condimentum sed, commodo vitae, ornare sit amet, wisi. Aenean fermentum, elit eget tincidunt condimentum, eros ipsum rutrum orci, sagittis tempus lacus enim ac dui.</p>
                </div>
                <div class="new section"></div>
              </div>
              <div class="parsys parsys1 twc-col2_6040-c1 parsys_column">
                <div class="parbase section">
                  <figure>
                    <img alt="important people at an important event" src="images/Formal_Affair.jpg">
                  </figure>
                </div>
                <div class="new section"></div>
              </div>
            </div>
            <div class="columnClear"></div>
          </div>

        </div>
      <div class="new section"></div>
      </div>
    </div>
    <div class="columnClear"></div>
  </div>
</div>

<div class="100 section columnControl">
  <div class="parsys_column twc-col1_100">
    <div class="parsys parsys0 twc-col1_100-c0 parsys_column">
      <div class="parbase section">
        <h3>Sed Ut Perspiciatis</h3>

        <div class="25-25-25-25 section columnControl">
          <div class="parsys_column twc-col4_25252525">

            <div class="parsys parsys0 twc-col4_25252525-c0 parsys_column">
              <div class="parbase section">
                <div class="bottom-box">
                  <figure>
                    <img alt="TWC_logo-placeholder" src="/TWC/core/images/TWC_logo-placeholder.png">
                  </figure>
                  <h5>People</h5>
                  <p>Liusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud</p>
                  <div class="cta omega twc-icon-after icon-angle-right mobile-full">
                              <a href="http://google.com" analyticsname="test">
                                  <span>Learn More</span>
                              </a>
                          </div>
                </div>
              </div>
              <div class="new section"></div>
            </div>

            <div class="parsys parsys1 twc-col4_25252525-c1 parsys_column">
              <div class="parbase section">
                <div class="bottom-box">
                  <figure>
                    <img alt="TWC_logo-placeholder" src="/TWC/core/images/TWC_logo-placeholder.png">
                  </figure>
                  <h5>Apps</h5>
                  <p>Liusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud</p>
                  <div class="cta omega twc-icon-after icon-angle-right mobile-full">
                              <a href="http://google.com" analyticsname="test">
                                  <span>Learn More</span>
                              </a>
                          </div>
                </div>
              </div>
              <div class="new section"></div>
            </div>

            <div class="parsys parsys1 twc-col4_25252525-c2 parsys_column">
              <div class="parbase section">
                <div class="bottom-box">
                  <figure>
                    <img alt="TWC_logo-placeholder" src="/TWC/core/images/TWC_logo-placeholder.png">
                  </figure>
                  <h5>TWC History</h5>
                  <p>Liusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud</p>
                  <div class="cta omega twc-icon-after icon-angle-right mobile-full">
                              <a href="http://google.com" analyticsname="test">
                                  <span>Learn More</span>
                              </a>
                          </div>
                </div>
              </div>
              <div class="new section"></div>
            </div>

            <div class="parsys parsys1 twc-col4_25252525-c3 parsys_column">
              <div class="parbase section">
                <div class="bottom-box">
                  <figure>
                    <img alt="TWC_logo-placeholder" src="/TWC/core/images/TWC_logo-placeholder.png">
                  </figure>
                  <h5>TWC Research</h5>
                  <p>Liusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud</p>
                  <div class="cta omega twc-icon-after icon-angle-right mobile-full">
                              <a href="http://google.com" analyticsname="test">
                                  <span>Learn More</span>
                              </a>
                          </div>
                </div>
              </div>
              <div class="new section"></div>
            </div>
          </div>
          <div class="columnClear"></div>
        </div>


      </div>
    <div class="new section"></div>
    </div>
  </div>
  <div class="columnClear"></div>
</div>

<div class="twc-box-column">
  <div class="100 section columnControl">
    <div class="parsys_column twc-col1_100">
      <div class="parsys parsys0 twc-col1_100-c0 parsys_column">
        <div class="parbase section">

          <div class="60-40 section columnControl">
            <div class="parsys_column twc-col2_6040">
              <div class="parsys parsys0 twc-col2_6040-c0 parsys_column">
                <div class="parbase section">
                  <h3>Headline for News Article</h3>
                  <p>Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. Donec eu libero sit amet quam egestas semper. Aenean ultricies mi vitae est. Mauris placerat eleifend leo. Quisque sit amet est et sapien ullamcorper pharetra. Vestibulum erat wisi, condimentum sed, commodo vitae, ornare sit amet, wisi. Aenean fermentum, elit eget tincidunt condimentum, eros ipsum rutrum orci, sagittis tempus lacus enim ac dui. <a href="">Tech Timeline</a></p>
                  <p>Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. Donec eu libero sit amet quam egestas semper. Aenean ultricies mi vitae est. Mauris placerat eleifend leo. Quisque sit amet est et sapien ullamcorper pharetra. Vestibulum erat wisi, condimentum sed, commodo vitae, ornare sit amet, wisi. Aenean fermentum, elit eget tincidunt condimentum, eros ipsum rutrum orci, sagittis tempus lacus enim ac dui.</p>
                </div>
                <div class="new section"></div>
              </div>
              <div class="parsys parsys1 twc-col2_6040-c1 parsys_column">
                <div class="parbase section">
                  <figure>
                    <img alt="important people at an important event" src="images/Formal_Affair.jpg">
                  </figure>
                </div>
                <div class="new section"></div>
              </div>
            </div>
            <div class="columnClear"></div>
          </div>

        </div>
      <div class="new section"></div>
      </div>
    </div>
    <div class="columnClear"></div>
  </div>
</div>

<div class="100 section columnControl">
  <div class="parsys_column twc-col1_100">
    <div class="parsys parsys0 twc-col1_100-c0 parsys_column">
      <div class="parbase section">
        <h3>Sed Ut Perspiciatis</h3>

        <div class="25-25-25-25 section columnControl">
          <div class="parsys_column twc-col4_25252525">

            <div class="parsys parsys0 twc-col4_25252525-c0 parsys_column">
              <div class="parbase section">
                <div class="bottom-box">
                  <figure>
                    <img alt="TWC_logo-placeholder" src="/TWC/core/images/TWC_logo-placeholder.png">
                  </figure>
                  <h5>People</h5>
                  <p>Liusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud</p>
                  <div class="cta omega twc-icon-after icon-angle-right mobile-full">
                              <a href="http://google.com" analyticsname="test">
                                  <span>Learn More</span>
                              </a>
                          </div>
                </div>
              </div>
              <div class="new section"></div>
            </div>

            <div class="parsys parsys1 twc-col4_25252525-c1 parsys_column">
              <div class="parbase section">
                <div class="bottom-box">
                  <figure>
                    <img alt="TWC_logo-placeholder" src="/TWC/core/images/TWC_logo-placeholder.png">
                  </figure>
                  <h5>Apps</h5>
                  <p>Liusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud</p>
                  <div class="cta omega twc-icon-after icon-angle-right mobile-full">
                              <a href="http://google.com" analyticsname="test">
                                  <span>Learn More</span>
                              </a>
                          </div>
                </div>
              </div>
              <div class="new section"></div>
            </div>

            <div class="parsys parsys1 twc-col4_25252525-c2 parsys_column">
              <div class="parbase section">
                <div class="bottom-box">
                  <figure>
                    <img alt="TWC_logo-placeholder" src="/TWC/core/images/TWC_logo-placeholder.png">
                  </figure>
                  <h5>TWC History</h5>
                  <p>Liusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud</p>
                  <div class="cta omega twc-icon-after icon-angle-right mobile-full">
                              <a href="http://google.com" analyticsname="test">
                                  <span>Learn More</span>
                              </a>
                          </div>
                </div>
              </div>
              <div class="new section"></div>
            </div>

            <div class="parsys parsys1 twc-col4_25252525-c3 parsys_column">
              <div class="parbase section">
                <div class="bottom-box">
                  <figure>
                    <img alt="TWC_logo-placeholder" src="/TWC/core/images/TWC_logo-placeholder.png">
                  </figure>
                  <h5>TWC Research</h5>
                  <p>Liusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud</p>
                  <div class="cta omega twc-icon-after icon-angle-right mobile-full">
                              <a href="http://google.com" analyticsname="test">
                                  <span>Learn More</span>
                              </a>
                          </div>
                </div>
              </div>
              <div class="new section"></div>
            </div>
          </div>
          <div class="columnClear"></div>
        </div>


      </div>
    <div class="new section"></div>
    </div>
  </div>
  <div class="columnClear"></div>
</div>


<?
if($_SERVER['REQUEST_URI'] !== '/TWC/styleguide/') {
  include($_SERVER['DOCUMENT_ROOT']. '/TWC/core/includes/footer.php');
}
?>
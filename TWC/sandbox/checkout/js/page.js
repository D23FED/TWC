...
// Min and Max Alert timeout - function is called in Compare Button Maximum Alert and Compare Button Min Alert
function minMaxTimeout() {
	$(".comparePackage").parent().siblings(".minimum, .maximum, .compareAble").fadeOut().removeClass('js-active');
}
// End Min and Max Alert Timeout - function is called in Compare Button Maximum Alert and Compare Button Min Alert
...
// Comparable Alert
$(".comparePackage").click(function() {
	var compareSelected = $(".comparePackage:checked").length;
	if ($(this).is(':checked')) {
		if (compareSelected >= 2) {
			$(this).parents('fieldset').siblings('.compareAble').toggle().toggleClass('js-active');
			setTimeout(minMaxTimeout, 5000);
		}
	}
});
// End Comparable Alert

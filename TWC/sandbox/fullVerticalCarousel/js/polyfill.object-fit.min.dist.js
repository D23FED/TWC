	...
	if ( "function" != typeof window.getMatchedCSSRules ) {
		var ELEMENT_RE = /[\w-]+/g,
			...
			getSpecificityScore = function( element, selectorText ) {
				for ( var selector, score, selectors = selectorText.split( "," ), result = 0; selector = selectors.shift(); ) _matchesSelector( element, selector ) && ( score = calculateScore( selector ), result = score > result ? score : result );
				return result
			},
			...
			customMatchesSelector = function( element, selector ) {
				for ( var matches = ( element.document || element.ownerDocument ).querySelectorAll( selector ), i = 0; matches[ i ] && matches[ i ] !== element; ) i++;
				return matches[ i ] ? !0 : !1
			},
			_matchesSelector = function( element, selector ) {
				var matcher = function( selector ) {
					return element.matches ? element.matches( selector ) : element.matchesSelector ? element.matchesSelector( selector ) : element.mozMatchesSelector ? element.mozMatchesSelector( selector ) : element.webkitMatchesSelector ? element.webkitMatchesSelector( selector ) : element.msMatchesSelector ? element.msMatchesSelector( selector ) : customMatchesSelector( element, selector )
				};
		...
		window.getMatchedCSSRules = function( element ) {
			...
			for ( styleSheets = toArray( window.document.styleSheets ); sheet = styleSheets.shift(); )
				for ( rules = getSheetRules( sheet ); rule = rules.shift(); ) rule.styleSheet ? rules = getSheetRules( rule.styleSheet ).concat( rules ) : rule.media ? rules = getSheetRules( rule ).concat( rules ) : _matchesSelector( element, rule.selectorText ) && result.push( rule );
...
function() {
	var lastTime = 0;
	window.requestAnimationFrame || ( window.requestAnimationFrame = window.webkitRequestAnimationFrame, window.cancelAnimationFrame = window.webkitCancelAnimationFrame || window.webkitCancelRequestAnimationFrame, window.requestAnimationFrame = function( callback ) {

    ...
    if(isIE(8) || isIE(9)){
        $(window).bind("load", function() {
            $('[class^=column-count]').each(function(event,i){
                ...
                $(this).columnize({ columns: splitClasses.slice(-1) }).show();
            });
...
$(window).resize(function() {
  ...
  if($(window).width() < 1059 && $(window).width() > 768){
    $(".sticky").css({ width : resizeStickyWidth + "px" });// 1059 is where the css for twc-container breaks the layout. 768 is where the problem fixes itself through css
  } else {
    $(".sticky").css({ width : "100" + "%" }); // Removes the width if you resize from tablet to mobile (user rotating phone from landscape to portrait)
  }
});

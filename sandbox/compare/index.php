<?php include($_SERVER['DOCUMENT_ROOT']. '/TWC/core/includes/header.php'); ?>
<h1>Compare TV Plans</h1>
<div class="backLink">
  <div class="cta icon-before icon-angle-left">
    <a href="#">
      <span>All TV Plans</span>
    </a>
  </div>
</div>
<div class="sbsCompare">
  <table class="triple">
    <thead class="sticky">
      <tr>
        <th class="single fs20">Phone</th>
        <th class="double fs20">TV, Internet</th>
        <th class="triple fs20">TV, Phone, Internet</th>
      </tr>
      <tr>
        <td class="text-center">
          <div class="pricing">
            <span class="h4">
              <sup class="pre-text">Add</sup>
                  <sup class="dollar-sign">$</sup>
              <span class="dollars">10</span>
              <sup class="cents">99</sup>
            </span>
            <span class="term">Per mo.<br>for 12 <a class="tooltip below" href="">mos.<span class="tooltip-text">Tolltip text</span></a></span>
          </div>
          <p>
            <div class="cta whiteCTA icon-before icon-angle-left">
              <a href="#">
                <span>Replace Plan</span>
              </a>
            </div>
          </p>
          <p>
            <div id="compare-package-1" class="cta orderCTA icon-after icon-angle-down">
              <a href="">
                <span>View &amp; Price Equipment</span>
              </a>
            </div>
          </p>
        </td>
        <td class="text-center">
          <div class="pricing">
            <span class="h4">
              <sup class="pre-text">Add</sup>
                  <sup class="dollar-sign">$</sup>
              <span class="dollars">44</span>
              <sup class="cents">99</sup>
            </span>
            <span class="term">Per mo.<br>for 12 <a class="tooltip below" href="">mos.<span class="tooltip-text">Tolltip text</span></a></span>
          </div>
          <p>
            <div class="cta whiteCTA icon-before icon-angle-left">
              <a href="#">
                <span>Replace Plan</span>
              </a>
            </div>
          </p>
          <p>
            <div id="compare-package-2" class="cta orderCTA icon-after icon-angle-down">
              <a href="#">
                <span>View &amp; Price Equipment</span>
              </a>
            </div>
          </p>
        </td>
        <td class="text-center">
          <div class="pricing">
            <span class="h4">
              <sup class="pre-text">Add</sup>
                  <sup class="dollar-sign">$</sup>
              <span class="dollars">109</span>
              <sup class="cents">99</sup>
            </span>
            <span class="term">Per mo.<br>for 12 <a class="tooltip below" href="">mos.<span class="tooltip-text">Tolltip text</span></a></span>
          </div>
          <p>
            <div class="cta whiteCTA icon-before icon-angle-left">
              <a href="#">
                <span>Replace Plan</span>
              </a>
            </div>
          </p>
          <p>
            <div id="compare-package-3" class="cta orderCTA icon-after icon-angle-down">
              <a href="#">
                <span>View &amp; Price Equipment</span>
              </a>
            </div>
          </p>
        </td>
      </tr>
    </thead>
    <tbody class="stickyElement">
      <tr class="stickySpacing">
        <td></td>
      </tr>
      <tr>
        <td>
          <div class="equipmentSelect">
            <p class="disclaimer-text text-center b">Disclaimer: One time charges, taxes, fees and surcharges are not included</p>
            <hr class="spacer">
            <p>TV Equipment</p>
            <div class="productSelect">
              <table>
                <tbody>
                  <tr>
                    <td>
                      <input id="tv-equipment-1" name="tv-equipment" type="radio" checked>
                    </td>
                    <td>
                      <label for="tv-equipment-1">
                        <a href="" class="tooltip above">HD Box
                          <span class="tooltip-text">This is a tooltip</span>
                        </a>
                      </label>
                    </td>
                    <td class="price">
                      <span>$10.25</span>
                    </td>
                  </tr>
                  <tr>
                    <td>
                      <input id="tv-equipment-2" name="tv-equipment" type="radio">
                    </td>
                    <td>
                      <label for="tv-equipment-2">
                        <a href="" class="tooltip above">HD-DVR Box and Service
                          <span class="tooltip-text">This is a tooltip</span>
                        </a>
                      </label>
                    </td>
                    <td class="price">
                      <span>$23.20</span>
                    </td>
                  </tr>
                  <tr>
                    <td>
                      <input id="tv-equipment-3" name="tv-equipment" type="radio">
                    </td>
                    <td>
                      <label for="tv-equipment-3">
                        <a href="" class="tooltip above">Enhanced DVR Box
                          <span class="tooltip-text">This is a tooltip</span>
                        </a>
                      </label>
                    </td>
                    <td class="price">
                      <span></span>
                    </td>
                  </tr>
                  <tr>
                    <td>
                      <input id="tv-equipment-4" name="tv-equipment" type="radio">
                    </td>
                    <td>
                      <label for="tv-equipment-4">
                        <a href="" class="tooltip above">Cablecard
                          <span class="tooltip-text">This is a tooltip</span>
                        </a>
                      </label>
                    </td>
                    <td class="price">
                      <span>$2.50</span>
                    </td>
                  </tr>
                </tbody>
              </table>
            </div>
            <hr>
            <p>Internet Equipment</p>
            <div class="productSelect">
              <table>
                <tbody>
                  <tr>
                    <td>
                      <input id="internet-equipment-1" name="internet-equipment" type="radio" checked="checked">
                    </td>
                    <td>
                      <label for="internet-equipment-1">
                        <a href="" class="tooltip above">Internet Modem
                          <span class="tooltip-text">This is a tooltip</span>
                        </a>
                      </label>
                    </td>
                    <td class="price">
                      <span>$8.00</span>
                    </td>
                  </tr>
                  <tr>
                    <td>
                      <input id="internet-equipment-2" name="internet-equipment" type="radio">
                    </td>
                    <td>
                      <label for="internet-equipment-2">
                        <a href="" class="tooltip above">Home WiFi and Wireless Gateway
                          <span class="tooltip-text">This is a tooltip</span>
                        </a>
                      </label>
                    </td>
                    <td class="price">
                      <span>$12.99</span>
                    </td>
                  </tr>
                  <tr>
                    <td>
                      <input id="internet-equipment-3" name="internet-equipment" type="radio">
                    </td>
                    <td>
                      <label for="internet-equipment-3">
                        <a href="" class="tooltip above">Wireless Gateway with Free Home WiFi
                          <span class="tooltip-text">This is a tooltip</span>
                        </a>
                      </label>
                    </td>
                    <td class="price">
                      <span>$8.00</span>
                    </td>
                  </tr>
                  <tr>
                    <td>
                      <input id="internet-equipment-4" name="internet-equipment" type="radio">
                    </td>
                    <td>
                      <label for="internet-equipment-4">
                        <a href="" class="tooltip above">I'll use my own modem
                          <span class="tooltip-text">This is a tooltip</span>
                        </a>
                      </label>
                    </td>
                    <td class="price">
                      <span>$0.00</span>
                    </td>
                  </tr>
                </tbody>
              </table>
            </div>
            <hr>
            <div class="clear"></div>
            <div class="priceTally">
              <table>
                <tr>
                  <td>Charges added</td>
                  <td>Equipment</td>
                  <td class="b">$18.25</td>
                </tr>
                <tr>
                  <td></td>
                  <td>Package</td>
                  <td class="b">$89.99</td>
                </tr>
                <tr>
                  <td>Charges reflected in your cart</td>
                  <td>Total</td>
                  <td class="b">$108.24</td>
                </tr>
              </table>
            </div>
            <div class="text-center">
              <p class="text-center">
                <a href="" class="tooltip above">Have everything you need?
                  <span class="tooltip-text">Before you finalize your order, you'll be able to review your selections to ensure you're happy with them.</span>
                </a>
              </p>
              <div class="cta whiteCTA icon-after icon-angle-right">
                <a href="https://www.timewarnercable.com/residential/order?qso=TESTDP8999">
                  <span>Express Checkout</span>
                </a>
              </div>
              <hr class="spacer">
              <p class="text-center">
                <a href="" class="tooltip above">Need more equipment or channels?
                  <span class="tooltip-text">Add additional set-top boxes, channels and more.</span>
                </a>
              </p>
              <div class="cta flatOmega icon-after icon-angle-right">
                <a href="https://www.timewarnercable.com/residential/order?qso=1-XB27P">
                  <span>Keep Shopping</span>
                </a>
              </div>
            </div>
          </div>
        </td>
        <td>
        </td>
        <td>
        </td>
      </tr>
      <tr>
        <th colspan="3">Special Offers</th>
      </tr>
      <tr>
        <td>
          <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Molestias, iure ab! Reiciendis cupiditate voluptatem ratione dicta blanditiis autem, architecto fuga.</p>
        </td>
        <td>
          <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Maxime tenetur soluta quam modi, ad! Numquam similique modi adipisci, itaque delectus quae, nihil voluptatibus, ex sed voluptatem consequuntur sapiente quia, laudantium accusamus. Nobis ullam mollitia, maxime excepturi, quod quo libero qui.</p>
        </td>
        <td>
          <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Maxime tenetur soluta quam modi, ad! Numquam similique modi adipisci, itaque delectus quae, nihil voluptatibus, ex sed voluptatem consequuntur sapiente quia, laudantium accusamus. Nobis ullam mollitia, maxime excepturi, quod quo libero qui. Lorem ipsum dolor sit amet, consectetur adipisicing elit. Voluptates, fuga.</p>
        </td>
      </tr>
      <tr class="subhead">
        <th colspan="3">TV</th>
      </tr>

      <tr>
        <th colspan="3">
          <a href="" class="tooltip above">Plans<span class="tooltip-text">Plan details here</span></a>
        </th>
      </tr>
      <tr>
        <td class="text-center">
          <div id="1" class="cta modal-button">
            <a href="">
              <span>200+ Channels</span>
            </a>
          </div>
          <div class="modal">
            <div class="modal-container" id="modal1">
              <div class="modal-close"></div>
              <div id="modal-content">
                <div class="modal-header">
                  <span class="h3">
                    <span class="twc-icon-before icon-exclamation-sign blue-icon"></span>
                    Over 200+ Channels
                  </span>
                </div>
                <div class="modal-body">
                  <div class="vertical">
                    <ul>
                      <li>1</li>
                      <li>2</li>
                      <li>3</li>
                      <li>4</li>
                      <li>5</li>
                      <li>6</li>
                      <li>7</li>
                      <li>8</li>
                      <li>9</li>
                      <li>10</li>
                    </ul>
                  </div>
                </div>
              </div>
            </div>
            <div class="modal-overlay"></div>
          </div>
        </td>
        <td class="text-center">
          <div id="2" class="cta modal-button">
            <a href="">
              <span>200+ Channels</span>
            </a>
          </div>
          <div class="modal">
            <div class="modal-container" id="modal2">
              <div class="modal-close"></div>
              <div id="modal-content">
                <div class="modal-header">
                  <span class="h3">
                    <span class="twc-icon-before icon-exclamation-sign blue-icon"></span>
                    Over 200+ Channels
                  </span>
                </div>
                <div class="modal-body">
                  <div class="vertical">
                    <ul>
                      <li>1</li>
                      <li>2</li>
                      <li>3</li>
                      <li>4</li>
                      <li>5</li>
                      <li>6</li>
                      <li>7</li>
                      <li>8</li>
                      <li>9</li>
                      <li>10</li>
                    </ul>
                  </div>
                </div>
              </div>
            </div>
            <div class="modal-overlay"></div>
          </div>
        </td>
        <td class="text-center">
          <div id="3" class="cta modal-button">
            <a href="">
              <span>200+ Channels</span>
            </a>
          </div>
          <div class="modal">
            <div class="modal-container" id="modal3">
              <div class="modal-close"></div>
              <div id="modal-content">
                <div class="modal-header">
                  <span class="h3">
                    <span class="twc-icon-before icon-exclamation-sign blue-icon"></span>
                    Over 200+ Channels
                  </span>
                </div>
                <div class="modal-body">
                  <div class="vertical">
                    <ul>
                      <li>1</li>
                      <li>2</li>
                      <li>3</li>
                      <li>4</li>
                      <li>5</li>
                      <li>6</li>
                      <li>7</li>
                      <li>8</li>
                      <li>9</li>
                      <li>10</li>
                    </ul>
                  </div>
                </div>
              </div>
            </div>
            <div class="modal-overlay"></div>
          </div>
        </td>
      </tr>
      <tr>
        <th colspan="3">Channels</th>
      </tr>
      <tr>
        <td>
          <p>Basic Internet w/ Up to 20Mbps</p>
        </td>
        <td>
          <p>Basic Internet w/ Up to 30Mbps</p>
        </td>
        <td>
          <p>Basic Internet w/ Up to 30Mbps</p>
        </td>
      </tr>
      <tr>
        <th colspan="3">HD Programming</th>
      </tr>
      <tr>
        <td>
          <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Molestias, iure ab! Reiciendis cupiditate voluptatem ratione dicta blanditiis autem, architecto fuga.</p>
        </td>
        <td>
          <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Maxime tenetur soluta quam modi, ad! Numquam similique modi adipisci, itaque delectus quae, nihil voluptatibus, ex sed voluptatem consequuntur sapiente quia, laudantium accusamus. Nobis ullam mollitia, maxime excepturi, quod quo libero qui.</p>
        </td>
        <td>
          <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Maxime tenetur soluta quam modi, ad! Numquam similique modi adipisci, itaque delectus quae, nihil voluptatibus, ex sed voluptatem consequuntur sapiente quia, laudantium accusamus. Nobis ullam mollitia, maxime excepturi, quod quo libero qui.</p>
        </td>
      </tr>
       <tr>
        <th colspan="3">DVR Service</th>
      </tr>
      <tr>
        <td class="text-center">
            <div class="icon-only icon-ok">
    <span></span>
  </div>
        </td>
        <td class="text-center">

        </td>
        <td class="text-center">
            <div class="icon-only icon-ok">
    <span></span>
  </div>
        </td>
      </tr>
      <tr>
        <th colspan="3">TWC TV</th>
      </tr>
      <tr>
        <td class="text-center">

        </td>
        <td class="text-center">
            <div class="icon-only icon-ok">
    <span></span>
  </div>
        </td>
        <td class="text-center">

        </td>
      </tr>
      <tr>
        <th colspan="3">Local News and Weather</th>
      </tr>
      <tr>
        <td class="text-center">
            <div class="icon-only icon-ok">
    <span></span>
  </div>
        </td>
        <td class="text-center">
            <div class="icon-only icon-ok">
    <span></span>
  </div>
        </td>
        <td class="text-center">
            <div class="icon-only icon-ok">
    <span></span>
  </div>
        </td>
      </tr>

      <!-- Internet -->
      <tr class="subhead">
        <th colspan="3">INTERNET</th>
      </tr>
          <tr>
        <th colspan="3"><a href="" class="tooltip above">Plans<span class="tooltip-text">Plan details here</span></a></th>
      </tr>
      <tr>
        <td>
          <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Molestias, iure ab! Reiciendis cupiditate voluptatem ratione dicta blanditiis autem, architecto fuga.</p>
        </td>
        <td>
          <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Maxime tenetur soluta quam modi, ad! Numquam similique modi adipisci, itaque delectus quae, nihil voluptatibus, ex sed voluptatem consequuntur sapiente quia, laudantium accusamus. Nobis ullam mollitia, maxime excepturi, quod quo libero qui.</p>
        </td>
        <td>
          <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Maxime tenetur soluta quam modi, ad! Numquam similique modi adipisci, itaque delectus quae, nihil voluptatibus, ex sed voluptatem consequuntur sapiente quia, laudantium accusamus. Nobis ullam mollitia, maxime excepturi, quod quo libero qui. Lorem ipsum dolor sit amet, consectetur adipisicing elit. Voluptates, fuga.</p>
        </td>
      </tr>
      <tr>
        <th colspan="3">Download Speed</th>
      </tr>
      <tr>
        <td>
          <p>Standard TV w/ Over 100 Channels</p>
        </td>
        <td>
          <p>Standard TV w/ Over 100 Channels</p>
        </td>
        <td>
          <p>Standard TV w/ Over 100 Channels</p>
        </td>
      </tr>
      <tr>
        <th colspan="3">Upload Speed</th>
      </tr>
      <tr>
        <td>
          <p>Basic Internet w/ Up to 20Mbps</p>
        </td>
        <td>
          <p>Basic Internet w/ Up to 30Mbps</p>
        </td>
        <td>
          <p>Basic Internet w/ Up to 30Mbps</p>
        </td>
      </tr>
      <tr>
        <th colspan="3">Home WiFi</th>
      </tr>
      <tr>
        <td>
          <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Molestias, iure ab! Reiciendis cupiditate voluptatem ratione dicta blanditiis autem, architecto fuga.</p>
        </td>
        <td>
          <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Maxime tenetur soluta quam modi, ad! Numquam similique modi adipisci, itaque delectus quae, nihil voluptatibus, ex sed voluptatem consequuntur sapiente quia, laudantium accusamus. Nobis ullam mollitia, maxime excepturi, quod quo libero qui.</p>
        </td>
        <td>
          <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Maxime tenetur soluta quam modi, ad! Numquam similique modi adipisci, itaque delectus quae, nihil voluptatibus, ex sed voluptatem consequuntur sapiente quia, laudantium accusamus. Nobis ullam mollitia, maxime excepturi, quod quo libero qui.</p>
        </td>
      </tr>
       <tr>
        <th colspan="3">TWC WiFi Hotspots</th>
      </tr>
      <tr>
        <td class="text-center">
            <div class="icon-only icon-ok">
    <span></span>
  </div>
        </td>
        <td class="text-center">

        </td>
        <td class="text-center">
            <div class="icon-only icon-ok">
    <span></span>
  </div>
        </td>
      </tr>
      <tr>
        <th colspan="3">Email Accounts</th>
      </tr>
      <tr>
        <td class="text-center">

        </td>
        <td class="text-center">
            <div class="icon-only icon-ok">
    <span></span>
  </div>
        </td>
        <td class="text-center">

        </td>
      </tr>
      <tr>
        <th colspan="3">Internet Security</th>
      </tr>
      <tr>
        <td class="text-center">
            <div class="icon-only icon-ok">
    <span></span>
  </div>
        </td>
        <td class="text-center">
            <div class="icon-only icon-ok">
    <span></span>
  </div>
        </td>
        <td class="text-center">
            <div class="icon-only icon-ok">
    <span></span>
  </div>
        </td>
      </tr>

      <!-- Phone -->
      <tr class="subhead">
        <th colspan="3">PHONE</th>
      </tr>
          <tr>
        <th colspan="3"><a href="" class="tooltip above">Plans<span class="tooltip-text">Plan details here</span></a></th>
      </tr>
      <tr>
        <td>
          <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Molestias, iure ab! Reiciendis cupiditate voluptatem ratione dicta blanditiis autem, architecto fuga.</p>
        </td>
        <td>
          <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Maxime tenetur soluta quam modi, ad! Numquam similique modi adipisci, itaque delectus quae, nihil voluptatibus, ex sed voluptatem consequuntur sapiente quia, laudantium accusamus. Nobis ullam mollitia, maxime excepturi, quod quo libero qui.</p>
        </td>
        <td>
          <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Maxime tenetur soluta quam modi, ad! Numquam similique modi adipisci, itaque delectus quae, nihil voluptatibus, ex sed voluptatem consequuntur sapiente quia, laudantium accusamus. Nobis ullam mollitia, maxime excepturi, quod quo libero qui. Lorem ipsum dolor sit amet, consectetur adipisicing elit. Voluptates, fuga.</p>
        </td>
      </tr>
      <tr>
        <th colspan="3">Included Calls</th>
      </tr>
      <tr>
        <td>
          <p>Standard TV w/ Over 100 Channels</p>
        </td>
        <td>
          <p>Standard TV w/ Over 100 Channels</p>
        </td>
        <td>
          <p>Standard TV w/ Over 100 Channels</p>
        </td>
      </tr>
      <tr>
        <th colspan="3">Calling Features</th>
      </tr>
      <tr>
        <td>
          <p>Basic Internet w/ Up to 20Mbps</p>
        </td>
        <td>
          <p>Basic Internet w/ Up to 30Mbps</p>
        </td>
        <td>
          <p>Basic Internet w/ Up to 30Mbps</p>
        </td>
      </tr>
      <tr>
        <th colspan="3">Enhanced 9-1-1</th>
      </tr>
      <tr>
        <td>
          <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Molestias, iure ab! Reiciendis cupiditate voluptatem ratione dicta blanditiis autem, architecto fuga.</p>
        </td>
        <td>
          <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Maxime tenetur soluta quam modi, ad! Numquam similique modi adipisci, itaque delectus quae, nihil voluptatibus, ex sed voluptatem consequuntur sapiente quia, laudantium accusamus. Nobis ullam mollitia, maxime excepturi, quod quo libero qui.</p>
        </td>
        <td>
          <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Maxime tenetur soluta quam modi, ad! Numquam similique modi adipisci, itaque delectus quae, nihil voluptatibus, ex sed voluptatem consequuntur sapiente quia, laudantium accusamus. Nobis ullam mollitia, maxime excepturi, quod quo libero qui.</p>
        </td>
      </tr>
       <tr>
        <th colspan="3">Voicemail</th>
      </tr>
      <tr>
        <td class="text-center">
            <div class="icon-only icon-ok">
    <span></span>
  </div>
        </td>
        <td class="text-center">

        </td>
        <td class="text-center">
            <div class="icon-only icon-ok">
    <span></span>
  </div>
        </td>
      </tr>
      <tr>
        <th colspan="3">Voicezone</th>
      </tr>
      <tr>
        <td class="text-center">

        </td>
        <td class="text-center">
            <div class="icon-only icon-ok">
    <span></span>
  </div>
        </td>
        <td class="text-center">

        </td>
      </tr>
      <tr>
        <th colspan="3">Global Penny Plan</th>
      </tr>
      <tr>
        <td class="text-center">
            <div class="icon-only icon-ok">
    <span></span>
  </div>
        </td>
        <td class="text-center">
            <div class="icon-only icon-ok">
    <span></span>
  </div>
        </td>
        <td class="text-center">
            <div class="icon-only icon-ok">
    <span></span>
  </div>
        </td>
      </tr>
    </tbody>
  </table>
</div>
<div class="text-center">
  <div class="cta fs-20 icon-after icon-angle-up scrollTop mobile-full">
    <a href="#">
      <span>Back to Top</span>
    </a>
  </div>
</div>

<hr class="spacer">
<p>Disclaimer Text - Lorem ipsum dolor sit amet, consectetur adipisicing elit. Itaque optio, eligendi rem illo, nostrum eos, aliquid atque doloribus sequi provident reprehenderit. Eius eaque eum illo, sit blanditiis tenetur doloribus harum similique nisi reiciendis perspiciatis aliquid nam modi pariatur autem tempore ab velit mollitia facere. Ad porro impedit non nemo, eligendi neque, dolorum cum maiores excepturi perferendis. Possimus, esse. Delectus corporis dicta eius excepturi quam expedita, odio, distinctio, officia perferendis facilis cum dolorem repellat magni aut consequatur fugit necessitatibus sapiente incidunt. Itaque dolorum impedit vero nesciunt magni cupiditate praesentium harum reiciendis, vitae nisi consequatur ratione sit, ipsam deleniti recusandae aliquam perspiciatis. Iste non aperiam in maxime quia odio rerum praesentium, perspiciatis amet consequatur eius voluptatem nesciunt ipsam similique neque cupiditate animi nisi, delectus odit minima autem totam! Magnam accusantium impedit tempora eveniet deserunt vitae dolorum blanditiis et repellat! Culpa veritatis quae quas accusamus quasi, tempore molestias maxime dolores doloribus perspiciatis, iste eaque neque natus dignissimos at eveniet corporis optio labore, esse ratione magnam necessitatibus reiciendis, quos nostrum. Sed ex provident possimus iure dolores quidem doloremque voluptatem porro eveniet nulla sit impedit inventore, numquam cupiditate, quod praesentium officia modi placeat quasi assumenda labore delectus accusamus. Iste rerum eveniet blanditiis minima vero repudiandae suscipit nihil cupiditate dolores nam accusantium eligendi voluptatem consectetur natus fugiat quae veritatis mollitia facilis magnam facere, culpa officiis, at nisi perspiciatis! Dolore harum eum possimus odio laudantium repellat quibusdam quas ratione quia amet, neque ad modi accusantium corporis sed sint similique aliquid voluptatibus ipsa, dolor veniam. Labore dignissimos, cupiditate!</p>


<?php include($_SERVER['DOCUMENT_ROOT']. '/TWC/core/includes/footer.php'); ?>
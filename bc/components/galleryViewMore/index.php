<?
if($_SERVER['REQUEST_URI'] !== '/TWC/bc/styleguide/') {
  include($_SERVER['DOCUMENT_ROOT']. '/TWC/bc/core/includes/header.php');
}
?>

<div class="gallery-viewMore">
    <div class="twc-box-column">
        <div class="search-bar twc-header-search twc-backgroundGradient-4">
            <div class="left"> <b>View More Gallery Subhead</b>
            </div>
            <div class="twc-module right mobile-full">
                <form action="<%= searchPage %>
                    ">
                    <fieldset class="twc-search">
                        <div class="twc-search-input-wrap">
                            <input type="search" placeholder="Search for Remote" name="search-q">
                            <div class="btn omega twc-search-btn">
                                <button type="submit"></button>
                            </div>
                        </div>
                    </fieldset>
                </form>
            </div>
        </div>
        <!-- Column Control 25/25/25/25 -->
        <div class="25-25-25-25 section columnControl">
            <div class="parsys_column twc-col4_25252525">
                <div class="parsys parsys0 twc-col4_25252525-c0 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="new section"></div>
                </div>
                <div class="parsys parsys1 twc-col4_25252525-c1 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="new section"></div>
                </div>
                <div class="parsys parsys1 twc-col4_25252525-c2 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="new section"></div>
                </div>
                <div class="parsys parsys1 twc-col4_25252525-c3 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="new section"></div>
                </div>
            </div>
            <div class="columnClear"></div>
        </div>
        <!-- .Column Control 25/25/25/25 -->
        <!-- Column Control 25/25/25/25 -->
        <div class="25-25-25-25 section columnControl">
            <div class="parsys_column twc-col4_25252525">
                <div class="parsys parsys0 twc-col4_25252525-c0 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="new section"></div>
                </div>
                <div class="parsys parsys1 twc-col4_25252525-c1 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="new section"></div>
                </div>
                <div class="parsys parsys1 twc-col4_25252525-c2 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="new section"></div>
                </div>
                <div class="parsys parsys1 twc-col4_25252525-c3 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="new section"></div>
                </div>
            </div>
            <div class="columnClear"></div>
        </div>
        <!-- Column Control 25/25/25/25 -->
        <div class="25-25-25-25 section columnControl">
            <div class="parsys_column twc-col4_25252525">
                <div class="parsys parsys0 twc-col4_25252525-c0 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>

                    </div>
                    <div class="new section"></div>
                </div>
                <div class="parsys parsys1 twc-col4_25252525-c1 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="new section"></div>
                </div>
                <div class="parsys parsys1 twc-col4_25252525-c2 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>

                    </div>
                    <div class="new section"></div>
                </div>
                <div class="parsys parsys1 twc-col4_25252525-c3 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="new section"></div>
                </div>
            </div>
            <div class="columnClear"></div>
        </div>
        <!-- .Column Control 25/25/25/25 -->
        <!-- Column Control 25/25/25/25 -->
        <div class="25-25-25-25 section columnControl">
            <div class="parsys_column twc-col4_25252525">
                <div class="parsys parsys0 twc-col4_25252525-c0 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="new section"></div>
                </div>
                <div class="parsys parsys1 twc-col4_25252525-c1 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="new section"></div>
                </div>
                <div class="parsys parsys1 twc-col4_25252525-c2 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="new section"></div>
                </div>
                <div class="parsys parsys1 twc-col4_25252525-c3 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="new section"></div>
                </div>
            </div>
            <div class="columnClear"></div>
        </div>
        <!-- .Column Control 25/25/25/25 -->
        <!-- Column Control 25/25/25/25 -->
        <div class="25-25-25-25 section columnControl">
            <div class="parsys_column twc-col4_25252525">
                <div class="parsys parsys0 twc-col4_25252525-c0 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>

                    </div>
                    <div class="new section"></div>
                </div>
                <div class="parsys parsys1 twc-col4_25252525-c1 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="new section"></div>
                </div>
                <div class="parsys parsys1 twc-col4_25252525-c2 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>

                    </div>
                    <div class="new section"></div>
                </div>
                <div class="parsys parsys1 twc-col4_25252525-c3 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="new section"></div>
                </div>
            </div>
            <div class="columnClear"></div>
        </div>
        <!-- .Column Control 25/25/25/25 -->
        <!-- Column Control 25/25/25/25 -->
        <div class="25-25-25-25 section columnControl">
            <div class="parsys_column twc-col4_25252525">
                <div class="parsys parsys0 twc-col4_25252525-c0 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="new section"></div>
                </div>
                <div class="parsys parsys1 twc-col4_25252525-c1 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="new section"></div>
                </div>
                <div class="parsys parsys1 twc-col4_25252525-c2 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="new section"></div>
                </div>
                <div class="parsys parsys1 twc-col4_25252525-c3 parsys_column">
                    <div class="parbase section">
                        <div class="remote-holder crop">
                            <a analyticsname="asdf:1:1:" href="#">
                                <figure>
                                    <img src="http://maxquattromani.com/TWC/bc/core/images/remote-1.png" alt"Remote" />
                                </figure>
                            </a>

                            <div class="blue-bar">
                                <div class="left">
                                    <a analyticsname="asdf:1:1:" href="#">
                                        <span>
                                            Atlas OCAP URC1056
                                        </span>
                                        <span>
                                            |
                                        </span>
                                    </a>
                                </div>
                                <div class="icon-info-sign icon-only">
                                    <div class="icon-hover">
                                        <a class="tooltip above" href="#">
                                            <span class="icon-info-sign icon-only"></span>
                                            <span class="tooltip-text">
                                                It's a hint! iis minus suscipit distinctio magni quas consequatur
                                            </span>
                                        </a>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="new section"></div>
                </div>
            </div>
            <div class="columnClear"></div>
        </div>
        <!-- .Column Control 25/25/25/25 -->

        <div class="content-footer">
            <div class="100 section columnControl">
                <div class="parsys_column twc-col1_100">
                    <div class="parsys parsys0 twc-col1_100-c0 parsys_column">
                        <div class="parbase section">
                            <div class="text-center">
                                <a class="viewMore" href="#">
                                    View More
                                </a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="new section"></div>

    <div class="text-center">
        <a class="viewMore" href="#">
            <span class="half-circle-bottom half-circle-bottom icon-only icon-angle-down">
                <span></span>
            </span>
        </a>
    </div>
</div>

<?
    if($_SERVER['REQUEST_URI'] !== '/TWC/bc/styleguide/') {
      include($_SERVER['DOCUMENT_ROOT']. '/TWC/bc/core/includes/footer.php');
  }
  ?>